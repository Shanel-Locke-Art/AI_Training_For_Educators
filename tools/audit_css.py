...
STYLE_MANIFEST = ROOT / "src/css/manifest.css"
RETIRED_CLASSES = {
    "name-modal-icon",
    "vn-main-menu-toggle",
    "s2-case-file--compact",
    "s2-case-clues",
    "s2-case-person",
    "s2-case-clue-heading",
    "s2-clue-strip",
    "s2-clue",
    "s2-loop-jordan",
    "s2-gap-intro",
    ...
    "pc-activity-layout",
    "pc-evidence-card",
    "pc-evidence-card-copy",
    "pc-button--secondary",
    "pc-activity-checkpoint",
    "pc-s2-engine-progress",
    "pc-s2-engine-note",
    "pc-s2-final",
    "pc-s2-final-header",
    "pc-s2-before-after",
    "pc-s2-comparison-card",
    "pc-s2-review-grid",
    "pc-s2-final-actions",
    "analysis-label-icon",
    "consult-babbage-btn",
...
STANDALONE_CSS = (ROOT / "src/css/pages/ideas-wall.css",)


def source_paths() -> tuple[Path, ...]:
    manifest = STYLE_MANIFEST.read_text(encoding="utf-8")
    values = re.findall(r'@import\s+url\(["\']([^"\'?]+)', manifest)
    return tuple(STYLE_MANIFEST.parent / value for value in values)
